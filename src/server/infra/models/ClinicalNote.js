...
class ClinicalNoteNotFoundError extends Error {
  constructor(message) {
    super(`Clinical note not found: ${message}`);
...
class ClinicalNote {
  ...
  /**
   * Find clinical note by ID within a tenant schema
   */
  static async findById(id, schema, includeSession = true) {
    let query = `
      ...
      LEFT JOIN public.users u ON cn.created_by_user_id_fk = u.id
      WHERE cn.id = $1
    `;
    ...
    if (result.rows.length === 0) {
      throw new ClinicalNoteNotFoundError(`ID: ${id} in schema: ${schema}`);
    }

    return new ClinicalNote(result.rows[0]);
  }

  /**
   * Find all clinical notes within a tenant schema
   */
  static async findAll(schema, options = {}) {
    ...
    if (sessionId) {
      conditions.push(`cn.session_id = $${params.length + 1}`);
      params.push(sessionId);
    }

    if (createdFrom) {
      // Accept ISO timestamp UTC from frontend (timezone-aware)
      conditions.push(`cn.created_at >= $${params.length + 1}::timestamptz`);
    ...
    if (createdByUserId) {
      conditions.push(`cn.created_by_user_id_fk = $${params.length + 1}`);
    ...
    return result.rows.map(row => new ClinicalNote(row));
  }

  /**
   * Count clinical notes within a tenant schema
   */
  static async count(schema, options = {}) {
    const { sessionId, patientId, createdByUserId } = options;

    let query = `SELECT COUNT(*) as count FROM ${schema}.clinical_note cn`;
    const params = [];
    const conditions = [];

    // Join with session if filtering by patientId
    if (patientId) {
      query += ` LEFT JOIN ${schema}.session s ON cn.session_id = s.id`;
    }
  ...
  /**
   * Create a new clinical note within a tenant schema
   */
  static async create(noteData, schema) {
    const { sessionId, content, createdByUserId = null } = noteData;

    const insertQuery = `
      INSERT INTO ${schema}.clinical_note (session_id, content, created_by_user_id_fk)
      VALUES ($1, $2, $3)
      RETURNING id
    `;
    ...
    // Fetch the created note with all joined data (session + patient + creator)
    const selectQuery = `
      SELECT cn.*, s.number as session_number, s.status as session_status, s.patient_id,
             p.first_name as patient_first_name, p.last_name as patient_last_name,
             p.email as patient_email, p.phone as patient_phone,
    ...
    const selectResult = await database.query(selectQuery, [noteId]);

    return new ClinicalNote(selectResult.rows[0]);
  }

  /**
   * Update an existing clinical note within a tenant schema
   */
  static async update(id, updates, schema) {
    ...
    // Fetch the updated note with all joined data (session + patient + creator)
    const selectQuery = `
      SELECT cn.*, s.number as session_number, s.status as session_status, s.patient_id,
             p.first_name as patient_first_name, p.last_name as patient_last_name,
             p.email as patient_email, p.phone as patient_phone,
  ...
  /**
   * Delete a clinical note within a tenant schema (hard delete)
   */
  static async delete(id, schema) {
    const query = `
      DELETE FROM ${schema}.clinical_note
      WHERE id = $1
      RETURNING *
    `;
...
module.exports = { ClinicalNote, ClinicalNoteNotFoundError };
