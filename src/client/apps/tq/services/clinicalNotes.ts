...
export interface ClinicalNote {
  id: string // UUID
  number: string // CLN000001, CLN000002, etc
  session_id: string
  content: string
  created_at: string
  updated_at: string
...
export const clinicalNotesService = {
  async list(params: ClinicalNotesListParams = {}): Promise<ClinicalNotesListResponse> {
    ...
    const url = `/api/tq/v1/clinical-notes${queryParams.toString() ? `?${queryParams.toString()}` : ''}`
  ...
  async getById(id: string): Promise<ClinicalNote> {
    const response = await api.get(`/api/tq/v1/clinical-notes/${id}`)
  ...
  async create(data: CreateClinicalNoteRequest): Promise<ClinicalNote> {
    const response = await api.post('/api/tq/v1/clinical-notes', data)
    // Defensive: handle both response.data.data and response.data formats
    return response.data.data || response.data
  },

  async update(id: string, data: UpdateClinicalNoteRequest): Promise<ClinicalNote> {
    const response = await api.put(`/api/tq/v1/clinical-notes/${id}`, data)
    // Defensive: handle both response.data.data and response.data formats
    return response.data.data || response.data
  },

  async delete(id: string): Promise<void> {
    await api.delete(`/api/tq/v1/clinical-notes/${id}`)
  }
}
